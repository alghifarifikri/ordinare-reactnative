...
import { getVegetarian } from '../../redux/action/Vegetarian'
...
class Vegetarians extends Component {
  constructor(props) {
    super(props)
    this.state = {
      isLoading : true,
  ...
  async componentDidMount() {
    await this.props.dispatch(getVegetarian())
  ...
  render() {
    ...
    <Container style = {{backgroundColor : '#0000FF'}}>
       <View style={{backgroundColor:"#0000FF", height:5}}></View>
        ...
        <FlatList
              data={this.props.vegetarian.data}
              renderItem={({item}) => (
...
const Vegetarian = withNavigation (Vegetarians)
...
export default connect (mapStateToProps) (Vegetarian)