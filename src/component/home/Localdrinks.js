...
import { getLocalDrinks } from '../../redux/action/Localdrinks'
...
class Localdrinkss extends Component {
  constructor(props) {
    super(props)
    this.state = {
      isLoading : true,
  ...
  async componentDidMount() {
    await this.props.dispatch(getLocalDrinks())
  ...
  render() {
    ...
    <Container style = {{backgroundColor : '#0000FF'}}>
       <View style={{backgroundColor:"#0000FF", height:5}}></View>
        ...
        <FlatList
              data={this.props.localdrinks.data}
              renderItem={({item}) => (
...
const Localdrinks = withNavigation (Localdrinkss)
const mapStateToProps = state =>{
  return{
    localdrinks : state.localdrinks,
  }
}

export default connect (mapStateToProps) (Localdrinks)